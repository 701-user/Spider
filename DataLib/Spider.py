...
class spider():
    ...
    def netTest(self):
        ...
        if net_state:
            print(
                datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " main " + "网络不连通，请连接网线  " + "运行状态: " + str(1))
    ...
    def init(self):
        ...
        try:
            ...
            self.dataBase.saveLogInfo("wxwb", "wxwb初始化失败", datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0)
            print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " wxwb " + "wxwb初始化失败  " + "运行状态: " + str(0))
        ...
        if os.path.exists(configDir):
            ...
            if not os.path.exists(AQIpath):
                ...
                print(datetime.datetime.now().strftime(
                    "%Y-%m-%d %H:%M:%S") + " main " + "配置文件夹丢失初始化失败，请确定配置文件AQI.ini是否存在  " + "运行状态: " + str(1))
            ...
            self.dataBase.saveLogInfo("main", "配置文件夹丢失初始化失败，请确定项目文件夹下配置文件夹config是否存在",
                                      datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 1)
            print(datetime.datetime.now().strftime(
    ...
    def wxwbInit(self):
        ...
        if weixinTime:
            ...
            # 时间间隔超过5天
            if day >= 10 and day <= 30:
                self.dataBase.saveLogInfo("main", "微信微博需要初始化一个月", datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                          1)
                print(datetime.datetime.now().strftime(
                    "%Y-%m-%d %H:%M:%S") + " main " + "微信微博需要初始化一个月  " + "运行状态: " + str(1))
                try:
                ...
                self.dataBase.saveLogInfo("main", "微信微博需要初始化四个个月",
    ...
    def run(self):
        ...
        while True:
            if self.netTest():
                if self.netFlag:
                    self.dataBase.saveLogInfo("main", "网络连通，所有线程唤醒",
                                              datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 1)
                    ## 判断微信微博中数据是否需要恢复
                    self.wxwbInit()
                    print(datetime.datetime.now().strftime(
                        "%Y-%m-%d %H:%M:%S") + " main " + "网络连通，所有线程唤醒  " + "运行状态: " + str(1))
                    ...
                    for i, spiderThread in enumerate(self.spiderThreadList):
                        if spiderThread.is_alive():
                            self.dataBase.saveLogInfo(spiderThread.getName(), "线程检测运行良好",
                                                      datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 1)
                            print(datetime.datetime.now().strftime(
                                "%Y-%m-%d %H:%M:%S") + " " + spiderThread.getName() + " 线程检测运行良好  " + "运行状态: " + str(1))
                        else:
                            ...
                            print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " " + self.spiderThreadList[
                                i].getName() + " 线程异常,重新启动  " + "运行状态: " + str(0))
                ...
                if not self.netFlag:
                    self.dataBase.saveLogInfo("main", "网络不通所有线程暂停",
                                              datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 1)
                    print(datetime.datetime.now().strftime(
                    ...
                    self.dataBase.saveLogInfo("main", "所有线程暂停中.....",
                                              datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 1)
                    print(datetime.datetime.now().strftime(
